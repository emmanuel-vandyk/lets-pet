      ...
      <section className='grid grid-flow-col w-full lg:justify-around items-center justify-center max-h-screen'>
        <article className='grid w-full '>
          <div className='flex w-full'>
            ...
            <h1 className='flex justify-center md:items-center md:justify-center w-full text-h1-semibold text-center font-extrabold whitespace-break-spaces'>Registrarse</h1>
          </div>
          <FormProvider {...methods}>
            <form onSubmit={handleSubmit(handleFormSubmit)} className='grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 lg:place-items-center w-fit gap-2 bg-default pt-4 pb-8 my-4 rounded-xl'>
              <FormItem className='px-10 md:mt-3'>
                <FormLabel className='text-lg after:content-["*"] after:ml-0.5 after:text-red-500'> Nombre </FormLabel>
                <Controller
                  control={control}
                  name='name'
                  render={({ field }) => <input {...field} className='flex items-center shadow-md p-1 outline-none' />}
                />
                {errors.name ? (
              ...
              <FormItem className='px-10 md:mt-3'>
                <FormLabel className='text-lg after:content-["*"] after:ml-0.5 after:text-red-500'> Apellido </FormLabel>
                <Controller
                  control={methods.control}
                  name='lastName'
                  render={({ field }) => <input {...field} className='flex items-start shadow-md p-1 outline-none' />}
                />
                {errors.lastName ? (
                  <span className='text-red-500 text-sm w-fit'> {errors.lastName.message} </span>
                ) : (
                  <span className='text-sm invisible'> Espacio reservado </span>
                )}
              </FormItem>
              ...
              <FormItem className='px-10'>
                <FormLabel className='text-lg after:content-["*"] after:ml-0.5 after:text-red-500'> Confirmar contraseña </FormLabel>
                <div className='relative'>
                  <Controller
                    control={methods.control}
                    ...
                    render={({ field }) => (
                      <div className="flex items-center justify-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => field.onChange(!!checked)} // Actualiza el estado
                        />
                        <label
                          htmlFor="terms"
